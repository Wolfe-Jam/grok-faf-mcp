...
exports.loadTechnicalCredit = loadTechnicalCredit;
exports.awardCredit = awardCredit;
exports.displayCreditDashboard = displayCreditDashboard;
exports.autoAwardCredit = autoAwardCredit;
 ...
 */
function getCreditCachePath() {
    const cacheDir = path_1.default.join(os_1.default.homedir(), '.faf-cli-cache');
    return path_1.default.join(cacheDir, 'technical-credit.json');
}
/**
 * Ensure cache directory exists
 */
async function ensureCacheDir() {
    const cacheDir = path_1.default.dirname(getCreditCachePath());
    try {
 ...
 */
async function loadTechnicalCredit(fafPath) {
    try {
        const cachePath = getCreditCachePath();
        const cacheData = await fs_1.promises.readFile(cachePath, 'utf-8');
        const allCredits = JSON.parse(cacheData);
        // Get credit for specific project or global
        const projectKey = fafPath ? path_1.default.resolve(fafPath) : 'global';
        ...
        credit.earned = credit.earned.map((source) => ({
            ...source,
 ...
 */
async function awardCredit(action, category, impact, fafPath) {
    try {
        ...
        const cacheDir = path_1.default.join(require('os').homedir(), '.faf-cli-cache');
        try {
            await fs_1.promises.access(cacheDir, fs_1.promises.constants.W_OK);
            // Only show achievement message if we have write access
            console.log(championship_style_1.FAF_COLORS.fafGreen(`${championship_style_1.FAF_ICONS.gem} ${championship_style_1.BRAND_MESSAGES.achievement} +${points} points`));
            console.log(championship_style_1.FAF_COLORS.fafCyan(`└─ ${impact}`));
        }
        catch {
 ...
 */
async function saveTechnicalCredit(credit, fafPath) {
    try {
        const cachePath = getCreditCachePath();
        // Load existing cache
        let allCredits = {};
        try {
            const existing = await fs_1.promises.readFile(cachePath, 'utf-8');
        ...
        // Save updated cache
        await fs_1.promises.writeFile(cachePath, JSON.stringify(allCredits, null, 2));
    }
    catch (error) {
 ...
 */
async function displayCreditDashboard(fafPath) {
    const credit = await loadTechnicalCredit(fafPath);
    console.log(championship_style_1.FAF_COLORS.fafCyan(`${championship_style_1.FAF_ICONS.gem} Technical Credit Dashboard`));
    ...
    console.log(`${championship_style_1.FAF_COLORS.fafCyan('├─ ')}Balance: ${championship_style_1.FAF_COLORS.fafGreen(`${credit.current} credits`)} ${trendingEmoji}`);
    // Credit level
    const level = getCreditLevel(credit.current);
    console.log(`${championship_style_1.FAF_COLORS.fafCyan('├─ ')}Level: ${level.name} ${level.emoji}`);
    // Recent earnings
    const recentEarnings = credit.earned
        .filter(s => Date.now() - s.timestamp.getTime() < 7 * 24 * 60 * 60 * 1000)
        .length;
    console.log(`${championship_style_1.FAF_COLORS.fafCyan('├─ ')}Recent Activity: ${recentEarnings} actions (7 days)`);
    // Top credit sources
    if (credit.earned.length > 0) {
        console.log(`${championship_style_1.FAF_COLORS.fafCyan('├─ ')}Top Sources:`);
        const topSources = credit.earned
            .slice(-3)
            .reverse();
        topSources.forEach(source => {
            const timeAgo = getTimeAgo(source.timestamp);
            console.log(`${championship_style_1.FAF_COLORS.fafCyan('│  ')}+${source.points} ${source.impact} (${timeAgo})`);
        });
    }
    console.log(championship_style_1.FAF_COLORS.fafCyan('└─────────────────────────────────'));
    ...
    if (credit.current >= 100) {
        console.log();
        console.log(championship_style_1.FAF_COLORS.fafGreen(`${championship_style_1.FAF_ICONS.trophy} ${championship_style_1.BRAND_MESSAGES.podium_celebration}`));
    }
    else if (credit.trending === 'up') {
        console.log();
        console.log(championship_style_1.FAF_COLORS.fafOrange(`${championship_style_1.FAF_ICONS.chart_up} Keep building credit! Next milestone: ${getNextMilestone(credit.current)} credits`));
    }
}
/**
 * Get credit level based on points
 */
function getCreditLevel(points) {
    if (points >= 200) {
 ...
 */
async function autoAwardCredit(trigger, fafPath) {
    const actions = {
        init_success: {
...
//# sourceMappingURL=technical-credit.js.map