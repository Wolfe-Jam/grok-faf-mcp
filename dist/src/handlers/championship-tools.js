        ...
        try {
            ...
            if (dir && dir.startsWith('/mnt/user-data/uploads/')) {
                ...
                if (detection.found && detection.path) {
                    ...
                    return await this.formatResult('🎯 FAF Auto-Path Detection', `${detection.error}\n\n` +
                        `**What I tried:**\n` +
...
⚠️  Root filesystem (/) is read-only - specify your project path!`;
            ...
            if (!result.success) {
                return await this.formatResult('🚀 FAF Init', `CLI Error: ${result.error}\n\nPlease ensure faf-cli v3.1.1+ is installed globally:\n  npm install -g faf-cli\n  or\n  brew install faf-cli`);
            }
        ...
        try {
            ...
            if (!args?.directory && !args?.projectName) {
                ...
                return await this.formatResult('⚡ FAF Quick - Create New Project', `**Enter a name for your project**\n\n` +
                    ...
                    `🎯 Projects-based: We organize your work in a Projects folder`, duration);
            }
    ...
    async handleSearch(args) {
        ...
        try {
            ...
            if (searchType === 'filename' || searchType === 'both') {
                ...
                if (matches.length === 0) {
                    return await this.formatResult('🔍 FAF Search', `No files found matching "${query}"\n\nSearched in: ${searchDir}`);
                }
                // Format results
            ...
            return await this.formatResult('🔍 FAF Search', 'Content search not yet implemented. Use type="filename" for now.');
        }
        catch (error) {
            return await this.formatResult('🔍 FAF Search Error', `Search failed: ${error.message}`);
        }
    }
...
Simple, fast, championship-grade.`;
    ...
    getScoreMedal(score) {
        ...
        if (score >= 85)
            return { medal: '🥉', status: 'Target 1 - Bronze' };
        if (score >= 70)
